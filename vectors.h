...
float SqrtInv (float x);            /// Быстрый корень квадратный
float SqrtInvE(float x);             /// Точный обратный корень
float Q_rsqrt2E( float number );     /// Быстрый обратный корень (не точный)

XYZ o_vector(XYZ A, XYZ B);             // обипцдеойе челфптб AB
XYZ vector_perp(XYZ a1, XYZ a2);        // ретреодйлхмст л челфптбн a1 Й a2
XYZ vector_ration(XYZ A);               // оптнбмйъбгйс челфптб
XYZ vector_normal(XYZ ab, XYZ ac);      // оптнбмш л челфптбн
XYZ vector_sim(XYZ A);                  // йънеоеойе птйеофбгйй челфптб
void vector_print(XYZ A);               // чщчпд челфптб
XYZ ve(XYZ A);                          // ччпд челфптб
double vector_SP(XYZ A, XYZ B);         // улбмстопе ртпйъчедеойе челфптпч
double vector_module(XYZ A);            // дмйоб челфптб
XYZ vector_sum(XYZ A, XYZ B);           // ухннб челфптпч
XYZ vector_YnC(XYZ A, double k);        // хнопцеойе челфптб об юйумп
XYZ vf(XYZ A);                          // ччпд челфптб
double vector_ygol(XYZ AB, XYZ AC);

XYZ vf(XYZ A, ifstream& file) // ВВОД ВЕКТОРА ИЗ ФАЙЛА
{
    file >> A.x;
    file >> A.y;
    file >> A.z;

...
*/
                        ...
                        RES2 = Ray_Trastd::cing_For_Picture(Obj_On_Scene.num[k], Person_Position, GDM_Picture[i][j]);
                        if(RES2 == 1){
                        ...
                        if(PARAM_Obvod == 1){
                            RES2 = Ray_Trastd::cing_For_Picture_Obvod(Obj_On_Scene.num[k], Person_Position, GDM_Picture[i][j]);
                            if(RES2 == 1){
...
int Ray_Trastd::cing_For_Picture(int Obj_Num, XYZ O, XYZ OV)   /// Определение пересечения прямой и треугольника
{
...
int Ray_Trastd::cing_For_Picture_Obvod(int Obj_Num, XYZ O, XYZ V)   /// Определение пересечения прямой и треугольника
{
//    Obj* D_Obj = (Obj*) pD_Obj;
    XYZ Peres;
    double D, mu;
...
//    }
        ...
        std::cout << "Прости чувак, но с одним потоком игра не запустится))" << std::endl;
        f_exit = 27;
    }
...
/**
            if(){**
                    ...
                    Ray_Trastd::cing функцию по каждой точке из блока (однако, можно просто вывести цвет близжайшего меша)
                **
...
void recursy(int j, int i, int dj, int di, Obj_Numeration_rec_Mass msObj)   //  msObj = mini scene Objects
{
    ...
    else if(di <= block.shirina || dj <= block.vysota){          /// вызов для каждого квадрата, Ray_Trastd::cing_For_Picture()
        int RES = 0;

        for(int pj = j; pj < j + dj; pj++){
            for(int pi = i; pi < i + di; pi++){ /// сперва лучше вызвать упорядочивание по расстоянию до объекта
                for(int num = 0; num < msObj.n; num++){
                    RES = Ray_Trastd::cing_For_Picture(msObj.Obj[num].num, Person_Position, GDM_Picture[pj][pi]);
    ...
    //        else{
                if(k == 0)
                    recursy(j, i, dj/2, di/2, dmsObj[0]);
                else if(k == 1)
                    recursy(j, i+(di/2), dj/2, di/2, dmsObj[1]);
                else if(k == 2)
                    recursy(j+(dj/2), i, dj/2, di/2, dmsObj[2]);
                else if(k == 3)
                    recursy(j+(dj/2), i+(di/2), dj/2, di/2, dmsObj[3]);
    //        }
        }
